...
The Patient Identity Source when grouped with ATNA Secure Node or Secure Application actor shall be able to record the following AuditEvents depending on the action requested:
...
The Patient Identifier Cross-reference Manager when grouped with ATNA Secure Node or Secure Application actor shall be able to record the following AuditEvents depending on the action performed:
- a [PIXm Feed Create Manager Audit Event Log](StructureDefinition-IHE.PIXm.Feed.Create.Audit.Manager.html) 
...
- a [PIXm Feed Update Manager Audit Event Log](StructureDefinition-IHE.PIXm.Feed.Update.Audit.Manager.html)
...
- a [PIXm Feed Delete Manager Audit Event Log](StructureDefinition-IHE.PIXm.Feed.Delete.Audit.Manager.html)
...
##### 2:3.104.5.2.1 When grouped with Basic Audit

The recording of the IUA token used should follow the as the following example shows
- [Audit Example for a PIXm Feed Create transaction from manager perspective](AuditEvent-ex-auditPixmFeed-create-manager.html).