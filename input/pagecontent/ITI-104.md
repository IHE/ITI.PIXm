...
A Patient Identity Source notifies the Patient Identifier Cross-reference Manager for [adding or revising patients](#2310441-add-or-revise-patient) in the Patient Identity Source.
The Patient Identity Source notifies the Patient Identifier Cross-reference Manager that [duplicate records](#2310442-resolve-patient-duplicate) were resolved in the Patient Identity Source.
...
The Patient Identifier Cross-reference Manger SHALL provide a CapabilityStatement with the [capabilities interaction](https://www.hl7.org/fhir/http.html#capabilities).
...
The Resolve Patient duplicate trigger signals that duplicate records were resolved in the Patient Identity Source.

A Resolve Patient duplicate message indicates that the Patient Identity Source has done a merge within a specific Patient Identification Domain. That is, the surviving patient identifier has subsumed a duplicate patient identifier.
...
The Patient Identifier Cross-reference Manager shall be capable of accepting attributes in the Duplicates Resolved message as specified in [2:3.104.4.2.2.1](#231044221-patient-resource-constraints).
...
Patient [Maiden Alice](Patient-Patient-MaidenAlice-Red.json.html) has been merged into [Patient Mohr Alice](Patient-Patient-MohrAlice-Red.json.html) in the 
Patient Identifier Domain urn:oid:1.3.6.1.4.1.21367.13.20.1000. The HTTP conditional update operation is done with the surviving patient identifier 'IHERED-994' from 
[Patient Mohr Alice](Patient-Patient-MohrAlice-Red.json.html) in the request and referencing the duplicate identifier 'IHERED-m94' of [Maiden Alice](Patient-Patient-MaidenAlice-Red.json.html) in the message:
...
Cross-reference Manager indicates in the [CapabilityStatement](http://hl7.org/fhir/capabilitystatement-definitions.html#CapabilityStatement.rest.resource.conditionalDelete) if this message is supported.
