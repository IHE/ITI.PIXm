...
The Resolve Duplicate Patient trigger signals that duplicate records were resolved in the Patient Identity Source.

A Resolve Duplicate Patient message indicates that the Patient Identity Source has done a merge within a specific Patient Identification Domain. That is, the surviving patient identifier has subsumed a duplicate patient identifier.
...
A link of type 'replaced-by' SHALL be added with the identifier pointing to the surviving patient identifier, see https://www.hl7.org/fhir/patient.html#dup-records.

##### 2:3.104.4.2.3 Expected Actions

The Patient Identifier Cross-reference Manger SHALL provide a CapabilityStatement with the [capabilities interaction](https://www.hl7.org/fhir/http.html#capabilities) and indicate that conditionalUpdate is available on the Patient.

The Patient Identifier Cross-reference Manager shall be capable of accepting attributes in the Resolve Duplicate Patient message as specified in [2:3.104.4.2.2.1](#231044221-patient-resource-constraints).

When the Patient Identifier Cross-reference Manager receives the Resolve Duplicate Patient message of the Patient Identity Feed transaction, it shall replace any references it is maintaining internally to the patient identifier included in the http request to the the patient identifier provided in the 'replaced-by' link. 
...
Patient Identifier Domain urn:oid:1.3.6.1.4.1.21367.13.20.1000. The HTTP conditional update operation is done with the duplicate identifier 'IHERED-m94' from 
